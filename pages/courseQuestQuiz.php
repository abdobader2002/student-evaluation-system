...
session_start();
include ('config.php');
// Fetch quiz_id from GET parameters
$quiz_id = isset($_GET['Quiz_id']) ? $_GET['Quiz_id'] : 0;

// Fetch course name (assuming you have a course_id in the quizzes table)
$course_name = '';
$sql_course = "SELECT courses.course_name FROM courses 
               JOIN quizzes ON courses.course_id = quizzes.course_id 
...
$sql = "SELECT questions.* FROM questions 
        JOIN question_quiz ON questions.question_id = question_quiz.question_id 
        WHERE question_quiz.quiz_id = '$quiz_id'";
        ...
        <div class="col-md-12 mt-4">
          <div class="card">
            <div class="card-header pb-0 px-3 d-flex justify-content-between">
              <h6 class="mb-0">
                <span
                  class="text-info text-gradient font-weight-bold"><?php echo htmlspecialchars($course_name); ?></span>
                Quizzes
              </h6>
            </div>
            <div class="card-body pt-4 p-3">
              <ul class="list-group">
                <?php if ($result->num_rows > 0) {
                  while ($row = $result->fetch_assoc()) { ?>
                    <li class="list-group-item border-0 d-flex p-4 mb-2 bg-gray-100 border-radius-lg">
                      <div class="d-flex flex-column">
                        <h6 class="mb-3 text-sm"><?php echo htmlspecialchars($row['question_text']); ?></h6>
                        <span class="mb-2 text-xs">Difficulty: <span
                            class="text-dark font-weight-bold ms-sm-2"><?php echo htmlspecialchars($row['difficulty']); ?></span></span>
                        <span class="mb-2 text-xs">Question Type: <span
                            class="text-dark font-weight-bold ms-sm-2"><?php echo htmlspecialchars($row['question_type']); ?></span></span>
                        <span class="mb-2 text-xs">Options: <span
                            class="text-dark ms-sm-2 font-weight-bold"><?php echo htmlspecialchars($row['options']); ?></span></span>
                        <span class="text-xs">Model Answer: <span
                            class="text-dark ms-sm-2 font-weight-bold"><?php echo htmlspecialchars($row['model_answer']); ?></span></span>
                      </div>
                  ...
                  <li class="list-group-item border-0 d-flex p-4 mb-2 bg-gray-100 border-radius-lg">
                    <div class="d-flex flex-column">
                      <h6 class="mb-3 text-sm">No questions found for this quiz.</h6>
                    </div>
                  </li>
                <?php } ?>
              </ul>
